...
def render_level4(user, supabase_client):
    ...
    st.markdown(
        ...
        '<tbody>' +
        '<tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Spoofing</td><td style="padding:10px 14px;color:#6B7280;">The tenant_id field is accepted directly from the client request with no server-side session verification (TRUST_CLIENT_TENANT_ID = True, VERIFY_TENANT_SESSION = False). Any authenticated client can claim any tenant identity simply by changing this field, with nothing to verify the claim matches their actual session.</td></tr>' +
        '<tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Tampering</td><td style="padding:10px 14px;color:#6B7280;">The Airflow fine-tuning pipeline pulls training data from its source with no integrity check (VALIDATE_DATA_INTEGRITY = False). A poisoned batch of advisory notes was able to reach the fine-tuning step unmodified-looking but tampered, embedding a hidden trigger phrase (AUDIT MODE: PAYGUARD-7741) directly into the model's weights.</td></tr>' +
        '<tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Repudiation</td><td style="padding:10px 14px;color:#6B7280;">Neither the RAG query layer nor the fine-tuning pipeline logs which identity actually issued a request or approved a training run — a spoofed cross-tenant query or an unauthorized data source in the pipeline leaves no attributable trail back to who did it.</td></tr>' +
